...
"""
Versão 8-bit com nomes de arquivos FIXOS em PT-BR, HUD e efeitos por tipo de coleta,
agora com MENSAGENS CONTEXTUAIS ao coletar cada item.
...
Rode: python3 coletaveis_sprites_colisao_8bit_fixos.py
"""
...
class Effect(pygame.sprite.Sprite):
    def __init__(self, pos):
        ...
        self.i = 0
        self.pos = pos
        self.image = make_pixel_surface(1,1)
        self.rect = self.image.get_rect(center=pos)
...
class PixelBurst(Effect):   # munição (bala): explosão de quadradinhos
...
class PixelPop(Effect):     # coração (vida): brilho quadrado "pop"
...
class PixelScan(Effect):    # bateria/peça: scanline horizontal
    def __init__(self, pos, steps=14, width=120, height=48):
        super().__init__(pos)
        self.frames = []
        W, H = width, height
        for t in range(steps):
...
class PixelRing(Effect):    # arma: anel quadrado expansivo
...
def main():
    ...
    msg_current = "Colete munição, comida e peças!"
    msg_timer = 0  # quando > 0, mostra mensagem contextual por alguns frames
    msg_map = {
        "municao": "Você coletou munição",
        "comida":  "Você coletou vida",
        "peca":    "Você coletou bateria",
        "arma":    "Você coletou arma",
    }
    MSG_DURATION = int(2.0 * FPS)  # ~2 segundos
    ...
    while running:
        ...
        for c in pygame.sprite.spritecollide(player, collectibles, False):
            ...
            msg_current = msg_map.get(c.kind, "Item coletado")
        ...
        # --- HUD Mensagem ---
        pygame.draw.rect(screen, (0, 0, 0, 120), (0, 0, screen.get_width(), 28))
        if msg_timer > 0:
            screen.blit(font.render(msg_current, True, WHITE), (10, 5))
            msg_timer -= 1
        pygame.display.flip()
